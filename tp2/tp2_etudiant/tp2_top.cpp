...
int _main(int argc, char *argv[])
{
    ...
    // Name, Base address, Size, Target, Cachable?
    // ROM
    ...
    segtable.addSegment("seg_kunc", SEG_KUNC_BASE, SEG_KUNC_SIZE, RAM_INDEX, false); // Uncacheable data
    segtable.addSegment("seg_code", SEG_CODE_BASE, SEG_CODE_SIZE, RAM_INDEX, true);  // Cacheable data
    ...
    for (size_t n = 0; n < ncycles; n++)
    {
        ...
        if (debug_ok && (n >= from_cycle))
        {
            ...
            std::cout << "req     = " << signal_req_proc.read() << std::endl
                      ...
                      << "lock    = " << signal_pi_lock.read() << std::endl
                      << std::hex
